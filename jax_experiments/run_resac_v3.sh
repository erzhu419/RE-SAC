#!/bin/bash
# ============================================================
# RE-SAC v3: Adaptive β + No Actor Weight Reg
# ============================================================
# Key changes from v2:
#   - Actor weight_reg REMOVED (was double-counting with critic target)
#   - Adaptive β_lcb: -2.0 → -0.5, warmup=20%
#   - β_ood=0.001, β_bc=0.0001 (same as v2)
#
# Results saved to resac_v3_{env}_8/
#
# Usage: bash jax_experiments/run_resac_v3.sh [cpu|gpu]
# ============================================================
...
echo "============================================================"
echo "  RE-SAC v3 (adaptive β, no actor weight_reg)"
echo "  β: ${BETA_START} → ${BETA_END}  warmup=${BETA_WARMUP}"
echo "  β_ood=$BETA_OOD  weight_reg=$WEIGHT_REG  β_bc=$BETA_BC"
echo "  Device: $DEVICE  Backend: $BACKEND  Seed: $SEED"
echo "============================================================"

PIDS=()
for env in "${ENVS[@]}"; do
    RUN_NAME="resac_v3_${env}_${SEED}"
    LOG_FILE="${LOG_DIR}/resac_v3_${env}_${SEED}.log"
...
echo "Monitor with: tail -f jax_experiments/logs/resac_v3_*_8.log"
echo ""
